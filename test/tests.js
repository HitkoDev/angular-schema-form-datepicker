  ...
  describe('directive', function() {
    ...
    it('should use datepicker directive when format is "date"', function() {
      ...
      inject(function($compile, $rootScope) {
        ...
        tmpl.children().eq(0).children().eq(1).is('div').should.be.true;
        tmpl.children().eq(0).children().eq(1).find('input[pick-a-date]').length.should.ok;
        tmpl.children().eq(0).children().eq(1).find('input[pick-a-date]').attr('max-date').should.be.ok;
        tmpl.children().eq(0).children().eq(1).find('input[pick-a-date]').attr('min-date').should.be.ok;
    ...
    it('should be disabled when readonly', function() {
      ...
      inject(function($compile, $rootScope) {
        ...
        $compile(tmpl)(scope);
        $rootScope.$apply();
        tmpl.children().eq(0).children().eq(1).find('input[pick-a-date]').attr('disabled').should.ok;
